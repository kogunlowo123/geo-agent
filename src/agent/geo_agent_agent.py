"""GEO Agent - Main Agent Implementation."""
...
class GeoAgentAgent:
    ...
    def __init__(self):
        ...
        self._tool_dispatch = {
            "audit_ai_visibility": self.tools.audit_ai_visibility,
            "optimize_for_llm": self.tools.optimize_for_llm,
            "track_ai_mentions": self.tools.track_ai_mentions,
            "generate_structured_data": self.tools.generate_structured_data,
            "benchmark_competitors": self.tools.benchmark_competitors,
        }
        logger.info(
            "agent_initialized",
            ...
            features=['geo', 'analytics', 'optimization'],
        )
    ...
    async def process_message(self, request: ChatRequest, user_id: str) -> ChatResponse:
        ...
        rag_results = await self.rag_pipeline.retrieve(
            ...
            top_k=5,
            filters={"user_id": user_id, "domain": "marketing"},
        )
    ...
    async def _maybe_execute_tools(self, message: str, context: str) -> list[dict]:
        ...
        tool_keywords = {
            "audit_ai_visibility": ['audit', 'ai', 'visibility'],
            "optimize_for_llm": ['optimize', 'for', 'llm'],
            "track_ai_mentions": ['track', 'ai', 'mentions'],
            "generate_structured_data": ['generate', 'structured', 'data'],
